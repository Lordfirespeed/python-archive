...
def genroom(x, y, entry):
    ...
    open_ways = [misc.bool_percent(60) for door in range(4)]
    open_ways_dict = {}
...
valid_commands = {}
with open("Resources/Usercommands.py", "r") as usercommandsfile:
    lines = usercommandsfile.readlines()
    lines = [line.strip() for line in lines]
    for index, line in enumerate(lines, 0):
        if line[:3] == "def":
            commandname = line[4:line.index("(")].title()
            precommands = []
            checklineindex = index + 1
            try:
                while lines[checklineindex][0] == "#":
                    precommands.append(lines[checklineindex][2:])
                    checklineindex += 1
            except IndexError:
                pass
            valid_commands[commandname] = precommands
...
while mainloop:
    ...
    while not moved:
        try:
            ...
            for precommand in valid_commands[user_input[0].title()]:
                exec(precommand)
