...
grid = [[int(value) for value in line.split(" ")] for line in stringgrid.split("\n")]
...
for y in range(len(grid)):
    for x in range(len(grid[0])-3):
        products.append(grid[y][x] * grid[y][x+1] * grid[y][x+2] * grid[y][x+3])

# get diagonals top left -> bottom right
for y in range(len(grid)-3):
    for x in range(len(grid[0])-3):
        products.append(grid[y][x] * grid[y+1][x+1] * grid[y+2][x+2] * grid[y+3][x+3])

# get diagonals top right -> bottom left
for y in range(len(grid)-3):
