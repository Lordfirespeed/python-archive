...
def getuser(known, value):
    ...
    return [str(c) for c in db.fetchone()]


def getuserfile(known, value):
    return "-".join(getuser(known, value)) + ".txt"
        ...
        if not loggedInA:
            ...
            while not chosen:
                ...
                if userChose not in validUsers:
                    ...
                    check = input("If you continue to log in, you will log out user '%s'. Continue? ('n' for no)\n> " % current)
                    chosen = "n" not in check.lower()

        if userChose == "a":
            print("Logging in as user A...")
            return ["loggedInA = %d" % userList[userIndex][0]]
        else:
            print("Logging in as user B...")
            return ["loggedInB = %d" % userList[userIndex][0]]
