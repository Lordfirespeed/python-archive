...
# If the program is main, open a connection to the DB - otherwise the caller will already have opened one
if __name__ == "__main__":
    conn = sqlite3.connect("database.db")
    db = conn.cursor()

userDirectory = r"C:\Users\joe\Google Drive\Misc\Master Python Project\Programming Projects\Card Game\Users\\"
...
# The message relating to the password constraints
...
def gettimenow():
    # Returns a string containing the current time in a readable format
    return datetime.now().strftime("%d-%m-%Y %H:%M")


def getusernames():
    db.execute("SELECT username FROM Users")
    # returns a list of usernames in the Users table
...
def getuser(known, value):
    # Returns the ID and username of a user based on either their ID or username, denoted by 'known'
    db.execute("SELECT ID, username FROM Users WHERE " + known + "=?", (value,))
    return db.fetchone()


def getuserfile(known, value):
    # Returns the expected filename for a given user
...
def validinput(msg, constraints):  # used to create new values, such as new users
    ...
    while not all(valid):
        ...
        # [print((con[0] % tuple([insertString for i in range(con[0].count("%s"))])  if con[0].count("%s") > 1
        ...
        valid = [eval(con[0] % tuple([insertString for i in range(con[0].count("%s"))]) if con[0].count("%s") > 1
                 else con[0] % insertString)
                 for con in constraints]
...
def validentry(listinst, desired):  # used for known data, such as logins
    lowerlist = [value.lower() for value in listinst]  # the function is passed a list of expected values - e.g. users
    correct = False
    while not correct:
        userInput = input("Please enter your %s:\n> " % desired)

        if userInput in listinst:  # the user has a 'perfect' input
            correct = True
        elif userInput.lower() in lowerlist:  # the user has a non-case-sensitive input
            didYouMean = listinst[lowerlist.index(userInput.lower())]
            # ask them whether they meant the value with different cases
            ...
            for checkName in lowerlist:
                if similar(userInput, checkName) > 0.6:  # if the entered value is similar to any of the valid values
                    didYouMean = listinst[lowerlist.index(checkName)]
                    # ask the user whether they meant the similar - and valid - values
                    check = input("Did you mean '%s'? ('n' to deny; any to confirm)\n> " % didYouMean)
                    correct = "n" not in check.lower()
                    if correct:
                        userInput = didYouMean
...
def selectuser(known, value):  # The user selects whether to login as A or B
    global loggedInA
    global loggedInB

    # if either user is not already logged in: auto-log-in as one of them
    if not loggedInA:
        userChose = "a"
    elif not loggedInB:
        userChose = "b"
        ...
        chosen = False
        while not chosen:
            # Ask the user who they want to log in as, as both users are already logged in
...
def login():
    ...
    try:
        ...
        if enteredUsername in (loggedInA, loggedInB):
            print("That user is already logged in. Cancelling...")
            return None
...
def formatgame(game):
    msg = game[0] + ": " + {"W": "Won", "L": "Lost"}[game[2]] + " with " + str(game[1]) + " cards"
...
def leaderboard():
    ...
    if getAmount == 0:
        ...
        for pick in range(getAmount):
            msg = "%d: %s won with %d cards on %s at %s"
            picked = (scoredGames[::-1])[pick]
            print(msg % (pick+1, picked[1], picked[0], picked[2][:picked[2].index(" ")], picked[2][picked[2].index(" ")+1:]))
