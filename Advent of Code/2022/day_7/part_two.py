...
class Solution:
    ...
    def size_of_smallest_directory_that_would_clear_enough_space(self) -> int:
        dir_sizes = {}
        total_space_taken_up = self.file_tree.root.calculate_size(dir_sizes)
        need_to_delete = self.needed_free_space - (self.total_device_space - total_space_taken_up)
        big_enough = [dir_size for dir_name, dir_size in dir_sizes.items() if dir_size >= need_to_delete]
...
if __name__ == "__main__":
    ...
    result = solver.size_of_smallest_directory_that_would_clear_enough_space()
    print(f"Size of smallest directory that would clear enough space if deleted: {result}")
