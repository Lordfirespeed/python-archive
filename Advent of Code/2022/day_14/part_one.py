...
class Solution:
    ...
    def __init__(self, input_lines: [str]) -> None:
        self.wall_strings = input_lines
        self.cave = Cave()
        self.generate_cave_walls()
        self.maximum_height_wall = max(self.cave.tiles.keys())
    ...
    def generate_cave_wall_segment(self, start: Vector2, end: Vector2) -> None:
        for position in self.line_of_vectors(start, end):
            self.cave[position] = TileState.Rock

    def generate_cave_wall(self, wall_string: str) -> None:
        ...
        for vector_pair in zip(vector_vertex_positions[:-1], vector_vertex_positions[1:]):
            self.generate_cave_wall_segment(*vector_pair)

    def generate_cave_walls(self) -> None:
        for wall_string in self.wall_strings:
            self.generate_cave_wall(wall_string)
    ...
    def spawn_sand(self) -> None:
        ...
        while previous_sand_position != current_sand_position:
            ...
            for fall_delta in self.sand_fall_deltas:
                ...
                break
            if current_sand_position.y > self.maximum_height_wall:
                raise SandInVoid
