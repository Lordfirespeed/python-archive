...
class Crane:
    ...
    def move_some_boxes(self, instruction: Crane.Instruction) -> None:
        from_stack = self.stacks[instruction.from_index]
        boxes = from_stack[-instruction.quantity:]
        self.stacks[instruction.from_index] = from_stack[:-instruction.quantity]
        self.stacks[instruction.to_index] += boxes

    def execute_instruction(self, instruction: str) -> None:
        instruction = self.Instruction.from_string(instruction)
        self.move_some_boxes(instruction)
