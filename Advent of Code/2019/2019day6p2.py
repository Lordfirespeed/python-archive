...
while changed:
    changed = False
    for orbit in orbits:
        if orbiting := base.recrgetchild(orbit[0]):
            if not orbiting.getchild(orbit[1]):
                orbiting.addchild(Tree(orbit[1], parent=orbiting))
                changed = True

santa = base.recrgetchild("SAN")
target = santa.getparent()
highest = target
moves = 0
while not highest.recrgetchild("YOU"):
    highest = highest.getparent()
    moves += 1

curr = base.recrgetchild("YOU").getparent()
while not curr == highest:
    curr = curr.getparent()
    moves += 1
