...
class Solution:
    ...
    def check_all_octopi_zero_energy(self):
        for octopusLine in self.octopusCavern:
            for octopus in octopusLine:
                if octopus.energy != 0:
                    return False
    ...
    def find_synchronised_flash_step(self):
        while not self.check_all_octopi_zero_energy():
...
if __name__ == "__main__":
    ...
    result = solver.find_synchronised_flash_step()
    print(f"Synchronised flash at step index: {result}")
