...
class Solution:
    ...
    @staticmethod
    def sum_of_snailfish_numbers(snailfishNumbers):
        sumResult = snailfishNumbers[0]
        for snailfishNumber in snailfishNumbers[1:]:
            sumResult = sumResult + snailfishNumber
        return sumResult

    def sum_all_snailfish_number_pairs(self):
        for snailfishNumberStrings in permutations(self.inputLines, 2):
            snailfishNumbers = [self.create_snailfish_number_object_from_string(line) for line in snailfishNumberStrings]
            self.sums.append(self.sum_of_snailfish_numbers(snailfishNumbers).magnitude())

    def get_largest_two_sum_magnitude(self):
        self.sum_all_snailfish_number_pairs()
...
if __name__ == "__main__":
    ...
    result = solver.get_largest_two_sum_magnitude()
